...
public class ViewFinderView extends View implements IViewFinder {
    ...
    @Override
    public synchronized void setLaserColor(int laserColor) {
        mLaserPaint.setColor(laserColor);
    }

    @Override
    public synchronized void setMaskColor(int maskColor) {
    ...
    @Override
    public synchronized void setBorderStrokeWidth(int borderStrokeWidth) {
        mBorderPaint.setStrokeWidth(borderStrokeWidth);
    }

    @Override
    public synchronized void setBorderLineLength(int borderLineLength) {
        mBorderLineLength = borderLineLength;
    }

    @Override
    public synchronized void setLaserEnabled(boolean isLaserEnabled) {
        mIsLaserEnabled = isLaserEnabled;
        postInvalidate();
    }

    @Override
    public synchronized void setBorderCornerRounded(boolean isBorderCornersRounded) {
    ...
    @Override
    public synchronized void setBorderAlpha(float alpha) {
    ...
    @Override
    public synchronized void setBorderCornerRadius(int borderCornersRadius) {
    ...
    public double getWidthRatio() {
        ...
        if (orientation != Configuration.ORIENTATION_PORTRAIT) {
            double finderWidth = (getHeight() * DEFAULT_SQUARE_DIMENSION_RATIO);
            int width = getWidth();
            return (finderWidth / width) * 100;
        } else {
            return DEFAULT_SQUARE_DIMENSION_RATIO * 100;
        }
    }
    ...
    public double getHeightRatio() {
        ...
        if (orientation != Configuration.ORIENTATION_PORTRAIT) {
            ...
            double finderWidth = (getWidth() * DEFAULT_SQUARE_DIMENSION_RATIO);
            int height = getHeight();
            return (finderWidth / height) * 100;
        }
    }
    ...
    public synchronized void drawViewFinderMask(Canvas canvas) {
    ...
    public synchronized void drawViewFinderBorder(Canvas canvas) {
        Rect framingRect = getFramingRect();
    ...
    public synchronized void drawLaser(Canvas canvas) {
        Rect framingRect = getFramingRect();
    ...
    @Override
    protected synchronized void onSizeChanged(int xNew, int yNew, int xOld, int yOld) {
        updateFramingRect();
    }
